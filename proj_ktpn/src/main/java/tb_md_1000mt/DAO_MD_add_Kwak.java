...
import testMM.v01_park.testSM_DTO;

public class DAO_MD_add_Kwak {
	...
	public List selectAllPCode() {
		...
		System.out.println("DAO_MD_add_Kwak selectAllPCode 실행");
		...
		try {
			...
			// SQL 준비
			String query = " select MT_MNG_CD, " + "	MT_MNG_NM " + " from TB_MD_1000MT where TP_CLS_VAL = '완제품' ";
...
//				ps.setString(1, dto.getTP_CLS_VAL());
			...
			while (rs.next()) {
				DTO_MD_add_Kwak dto = new DTO_MD_add_Kwak();
	...
	public List selectAllMCode() {
		...
		System.out.println("DAO_MD_add_Kwak selectAllMCode 실행");
		...
		try {
			...
			// SQL 준비
			String query = " select MT_MNG_CD, " + "	MT_MNG_NM " + " from TB_MD_1000MT where TP_CLS_VAL = '원자재' ";
